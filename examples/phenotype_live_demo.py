...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='phenotype-browser-', ignore_cleanup_errors=True) as home:
        ...
        try:
            ...
            browser('open', args.url)
            browser('wait', '#run')
            browser('js', 'document.querySelector("#human-message").value="Help diagnose this patient from their symptoms and available genetic data. Explain the evidence and respect the data access restrictions."')
            browser('js', 'document.querySelector("#terms").value="Ectopia lentis (HP:0001083); Arachnodactyly (HP:0001166); Aortic root aneurysm (HP:0002616)"')
            browser('click', '#run')
            ...
            assert manual['human_message'].startswith('Help diagnose this patient')
            ...
            browser('js', 'document.querySelector("#terms").value="Ectopia lentis (HP:0001166)"')
            browser('click', '#run')
            wait('document.querySelector("#error").textContent.includes("label/ID mismatch") && !document.querySelector("#run").disabled')
            browser('assert', 'document.querySelectorAll("#rows tr").length===0')
            browser('js', 'document.querySelector("#terms").value="arachnodactyly (MP:0006296)"')
            browser('click', '#run')
            wait('document.querySelectorAll("#rows tr").length===20 && !document.querySelector("#run").disabled')
