"""Verify the public live demo in a private browser; submits a new local analysis and DDBJ job."""
import argparse
import os
import subprocess
import tempfile
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='public-replay-browser-', ignore_cleanup_errors=True) as home:
        ...
        try:
            ...
            browser('assert', 'document.body.textContent.includes("Live visitor session")')
            run();inspect(510)
            ...
            assert browser('js', '(async()=>(await (await fetch("/api/demo")).json()).run.id)()')==first
            print('Live visitor: real DDBJ analysis, independent reset and three aggregate rows passed.', flush=True)
            if args.screenshot:
                browser('open', args.url)
